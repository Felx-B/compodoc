...
var Dependencies = (function () {
                ...
                var visitNode = function (visitedNode, index) {
                    ...
                    if (_this.isModule(metadata)) {
                        ...
                        if (_this.configuration.mainData.publicDocumentation) {
                            _this.filterComponentContent(deps, node);
    ...
    Dependencies.prototype.getAllowedList = function (node) {
        var allowed = [];
        for (var _i = 0, _a = node.members; _i < _a.length; _i++) {
            var member = _a[_i];
            var name = (member.symbol && member.symbol.name) || (member.name && member.name.text);
            if (name == "AllowedMembers") {
                var children = member.initializer.properties;
                for (var _b = 0, children_1 = children; _b < children_1.length; _b++) {
                    ...
                    if (child.name.text == "Proxy" || child.name.text == "Config") {
                        for (var _c = 0, _d = child.initializer.elements; _c < _d.length; _c++) {
                            var prop = _d[_c];
    ...
    Dependencies.prototype.filterComponentContent = function (deps, node) {
        var allowedList = this.getAllowedList(node);
        ...
        for (var _i = 0, _a = deps.propertiesClass; _i < _a.length; _i++) {
            var prop = _a[_i];
            for (var _b = 0, allowedList_1 = allowedList; _b < allowedList_1.length; _b++) {
                var allowedItem = allowedList_1[_b];
                if (prop.name == allowedItem)
                    filteredProperties.push(prop);
            }
        }
        for (var _c = 0, _d = deps.methodsClass; _c < _d.length; _c++) {
            var method = _d[_c];
            for (var _e = 0, allowedList_2 = allowedList; _e < allowedList_2.length; _e++) {
                var allowedItem = allowedList_2[_e];
                if (method.name == allowedItem)
                    filteredMethods.push(method);
            }
        }
        for (var _f = 0, _g = deps.outputsClass; _f < _g.length; _f++) {
            var output = _g[_f];
            for (var _h = 0, allowedList_3 = allowedList; _h < allowedList_3.length; _h++) {
                var allowedItem = allowedList_3[_h];
